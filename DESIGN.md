...
- **Wallpaper**: a static night-sky nebula, no WebGL and no shader. The nebula is plain CSS (`.glass body` in `index.css`) — a muted plum cloud upper-left, a muted teal cloud upper-right, and a faint warm dust lane, over a near-black charcoal base (`#030408`); it never animates, so it costs nothing. On top, `GlassBackdrop` mounts a `Starfield` layer (`client/src/lib/starfield/`, Canvas2D, into the fixed `.liquid-field` layer): ~90–340 stars (scaled to viewport area) at positions generated once and never moved. The only motion is a per-star opacity twinkle, plus an occasional brighter flare pulse on some stars; the brightest ~4% render as a crisp core with thin cross-shaped diffraction spikes rather than a larger or blurred disc, so bright stars read as points of light, not glowing orbs. Capped at 20fps (twinkle doesn't need more) with a device-pixel-ratio cap of 1.5. Reduced motion renders a single settled frame; a hidden tab pauses the loop entirely. This replaced three earlier attempts, in order: a five-layer raymarched WebGL2 scene (glass sculpture, aurora, bloom) that stalled even recent integrated GPUs; a WebGL2 "Data Current" shader with colored light fields, which fixed the performance problem but still read as a saturated blob; and a toned-down version of the same shader, which fixed the saturation but was still a moving glowing shape and still too blue. The lesson that stuck: a glass theme's backdrop should be *material*, not a scene — nothing on it needs to move except individual points of light. `?theme=glass|dark|light` deep-links a theme (used by headless design QA).
