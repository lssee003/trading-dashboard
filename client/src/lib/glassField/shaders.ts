/**
 * GLSL for the Glass Field backdrop, adapted from the Claude Design file
 * "Glass Field.dc.html" (raymarched glass blob removed). Two fragment
 * programs drive a full-screen triangle:
 *   SCENE_FS — a starfield sky with aurora curtains and a drifting warm
 *              sun-star. Renders into an HDR (RGBA16F) offscreen target.
 *   COMP_FS  — tonemap pass: golden-angle bloom on highlights, ACES, black
 *              point, gentle contrast, cool vignette, filmic grain.
 * Retune via config.ts uniforms, not by editing GLSL.
 */
